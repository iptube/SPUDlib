...
CTEST(ls_htable, hash_strcase)
{
    ...
    ASSERT_EQUAL(ls_strcase_hashcode("Key1"), ls_strcase_hashcode("key1"));
    ASSERT_EQUAL(ls_strcase_hashcode("KEY1"), ls_strcase_hashcode("key1"));
    ASSERT_NOT_EQUAL(ls_strcase_hashcode("KEY_ONE"), ls_strcase_hashcode("key1"));
    ASSERT_NOT_EQUAL(ls_strcase_hashcode(""), ls_strcase_hashcode("key1"));
...
CTEST(ls_htable, basics)
{
    ...
    pvalue = NULL;
    ASSERT_TRUE(ls_htable_put(table, "key1", "value one", test_htable_store_pvalue, &err));
    ASSERT_NULL(pvalue );
    ASSERT_EQUAL(ls_htable_get_count(table) , 1);
    ASSERT_EQUAL(strcmp(ls_htable_get(table, "key1"), "value one") , 0);
    ASSERT_NULL(ls_htable_get(table, "key2") );

    ASSERT_TRUE(ls_htable_put(table, "key2", "value two", test_htable_store_pvalue, &err));
    ASSERT_NULL(pvalue );
    ASSERT_EQUAL(ls_htable_get_count(table) , 2);
    ASSERT_EQUAL(strcmp(ls_htable_get(table, "key1"), "value one") , 0);
    ASSERT_EQUAL(strcmp(ls_htable_get(table, "key2"), "value two") , 0);

    ASSERT_TRUE(ls_htable_put(table, "key1", "val 1", test_htable_store_pvalue, &err));
    ...
    pvalue = NULL;
    ASSERT_TRUE(ls_htable_put(table, "key1", "first value", test_htable_store_pvalue, &err));
    ASSERT_NULL(pvalue );
    ASSERT_EQUAL(ls_htable_get_count(table) , 2);
...
CTEST(ls_htable, destroy_key)
{
    ...
    key_destroy = false;
    char *k = strdup("key1");
    ASSERT_TRUE(ls_htable_put(table, k, strdup("value one"), test_htable_destroy_key, &err));
    ASSERT_FALSE(key_destroy);
    ASSERT_TRUE(ls_htable_put(table, k, strdup("value two"), test_htable_destroy_key, &err));
    ASSERT_FALSE(key_destroy);
    ASSERT_TRUE(ls_htable_put(table, strdup(k), strdup("value two"), test_htable_destroy_key, &err));
