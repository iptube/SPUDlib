...
CTEST2(ls_pktinfo, dup_fail)
{
    ...
    OOM_TEST_INIT()
    OOM_TEST_CONDITIONAL_CHECK(&data->err, ls_pktinfo_dup(data->pi, &pi, &data->err), true);
}
...
CTEST2(ls_pktinfo, get_addr)
{
    ...
    ASSERT_FALSE(ls_pktinfo_get_addr(data->pi, (struct sockaddr*)&ss, &ss_sz, &data->err));
    ...
    ss_sz = sizeof(ss);
    ASSERT_TRUE(ls_pktinfo_get_addr(data->pi, (struct sockaddr*)&ss, &ss_sz, &data->err));
