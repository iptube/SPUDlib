...
{
  ASSERT_TRUE( tube_manager_bind_event(data->mgr, EV_LOOPSTART_NAME, test_cb,
                                       &data->err) );
  ASSERT_TRUE( tube_manager_bind_event(data->mgr, EV_RUNNING_NAME, test_cb,
                                       &data->err) );
  ASSERT_TRUE( tube_manager_bind_event(data->mgr, EV_DATA_NAME, test_cb,
                                       &data->err) );
  ASSERT_TRUE( tube_manager_bind_event(data->mgr, EV_CLOSE_NAME, test_cb,
                                       &data->err) );
  ASSERT_TRUE( tube_manager_bind_event(data->mgr, EV_ADD_NAME, test_cb,
                                       &data->err) );
  ASSERT_TRUE( tube_manager_bind_event(data->mgr, EV_REMOVE_NAME, test_cb,
                                       &data->err) );
}
...
CTEST2(tube, open)
{
  ...
  ASSERT_TRUE( tube_manager_open_tube(data->mgr,
                                      (const struct sockaddr*)&remoteAddr, &t,
                                      &data->err) );
...
CTEST2(tube, send_pdec)
{
  ...
  ASSERT_TRUE( path_mandatory_keys_create(ip, sizeof(ip), token, sizeof(token),
                                          url, &ctx, &map, &data->err) );
