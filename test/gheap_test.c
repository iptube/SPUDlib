...
static int
timeval_less(const void* const context,
             const void* const a,
             const void* const b)
{
...
CTEST(gpriority_queue, basic)
{
  ...
  struct gpriority_queue* pq = gpriority_queue_create(&paged_binary_heap_ctx,
                                                      nodel);
