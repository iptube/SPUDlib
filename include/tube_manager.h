...
typedef enum {
  /**
   * Ignore the source IP/port, and just match on tube ID's.   Not currently
   * recommended.
   */
...
tube_manager_create(int            buckets,
                    ...
                    ls_err*        err);
/**
 * Shut down and deallocate a tube manager.  All associated data structures are
 * freed.
 *
 ...
 */
LS_API void
tube_manager_set_data(tube_manager* mgr,
                      void*         data);
 ...
 */
LS_API void*
tube_manager_get_data(tube_manager* mgr);
...
/**
 * Start receiving SPUD packets and parsing them. Queue events
 * (DATA/CLOSE/RUNNING) as appropriate.
 *
 * \invariant mgr != NULL, t != NULL
 ...
 *
 * \param[in]  mgr     The manager to schedule on
 * \param[in]  tv      The time (in seconds since the UTC epoch) to fire the
 * callback
 ...
 * \param[out] err    err If non-NULL on input, contains error if false is
 * returned
 * \return     true: callback was registered.  false: see err.
 */
LS_API bool
...
/**
 * Set the default functions that handle sending and receiving for all tubes,
 * i.e.,
